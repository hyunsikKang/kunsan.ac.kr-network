package kr.ac.kunsan.network.chatting._3.nio.nonblocking.server;
...
public class ChattingServer {
	...
	private void readHandle(SelectionKey key) throws IOException, ClassNotFoundException {
		try {
			...
			ChattingRequest chattingRequest = SocketRequestResponseUtils.getChattingRequest(channel, buffer);
			...
			if (type == ChattingRequest.MessageType.JOIN) {
				if (registClient(nickName, channel)) {
					...
					SocketRequestResponseUtils.writeResponse(response, channel);
					return;
				}
	...
	public void sendClients(ChattingResponse response) {
		for (SocketChannel socketChannel: clientMap.keySet()) {
			try {
				SocketRequestResponseUtils.writeResponse(response, socketChannel);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
